...
$platformCss = file_get_contents($root . '/assets/css/platform.css');
...
$assert($header !== false && $platform !== false && $theme !== false && $platformCss !== false, 'App shell files must be readable.');
...
$assert(str_contains($platform, 'assets/css/platform.css'), 'Platform console must load platform-only CSS outside the global app theme.');
...
foreach ([
    ...
    $assert(str_contains($platformCss, $needle), 'assets/css/platform.css missing platform visual contract: ' . $needle);
}
