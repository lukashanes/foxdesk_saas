...
$assert(str_contains($iosSettingsView, '.navigationTitle("Account")'), 'iOS account surface must use Account as the visible navigation title.');
$assert(str_contains($iosSettingsView, 'Privacy Policy'), 'iOS Account must link to privacy policy.');
$assert(str_contains($iosSettingsView, 'Contact support'), 'iOS Account must include support contact.');
$assert(str_contains($iosSettingsView, 'Request account deletion'), 'iOS Account must include data deletion request link.');
$assert(str_contains($iosSettingsView, 'mailto:support@foxdesk.net?subject=FoxDesk%20iOS%20support'), 'iOS Account support must use the FoxDesk support mailto link.');
$assert(str_contains($iosSettingsView, 'mailto:support@foxdesk.net?subject=FoxDesk%20account%20deletion%20request'), 'iOS Account account deletion must use a dedicated support deletion request link.');
$assert(str_contains($iosSettingsView, 'https://foxdesk.net/index.php?page=legal&type=privacy'), 'iOS Account privacy link must point to the live FoxDesk privacy document.');
$assert(str_contains($iosSettingsView, 'https://foxdesk.net/index.php?page=legal&type=terms'), 'iOS Account terms link must point to the live FoxDesk terms document.');
...
$assert(str_contains($iosSettingsView, 'Push diagnostics'), 'iOS Account must include debug/staging APNs diagnostics for physical-device smoke testing.');
$assert(str_contains($iosSettingsView, 'session.client.environment.baseURL.absoluteString'), 'iOS debug Account must show the active API base URL for TestFlight/backend verification.');
$assert(str_contains($iosSettingsView, 'Copy APNs token'), 'iOS Account must let testers copy the APNs token for live smoke tests.');
$assert(str_contains($iosSettingsView, 'UIPasteboard.general.string'), 'iOS Account must copy the APNs token to the clipboard.');
$assert(str_contains($iosSettingsView, '.textSelection(.enabled)'), 'iOS Account must make APNs token text selectable.');
