...
$iosSettingsView = file_get_contents($root . '/ios/FoxDesk/FoxDesk/Sources/SettingsView.swift');
...
$assert($iosSettingsView !== false, 'iOS Settings view is missing.');
...
$assert(str_contains($iosTestFlightRunbook, 'Settings → Push diagnostics'), 'iOS TestFlight runbook must document how to capture a physical-device APNs token.');
...
$assert(str_contains($nativeDocs, 'Settings → Push diagnostics'), 'Native API docs must document the iOS APNs token capture path.');
...
$assert(str_contains($iosSettingsView, 'Privacy Policy'), 'iOS Settings must link to privacy policy.');
$assert(str_contains($iosSettingsView, 'Contact support'), 'iOS Settings must include support contact.');
$assert(str_contains($iosSettingsView, 'Request account deletion'), 'iOS Settings must include data deletion request link.');
$assert(str_contains($iosSettingsView, 'mailto:support@foxdesk.net?subject=FoxDesk%20iOS%20support'), 'iOS Settings support must use the FoxDesk support mailto link.');
$assert(str_contains($iosSettingsView, 'mailto:support@foxdesk.net?subject=FoxDesk%20account%20deletion%20request'), 'iOS Settings account deletion must use a dedicated support deletion request link.');
$assert(str_contains($iosSettingsView, 'https://foxdesk.net/index.php?page=legal&type=privacy'), 'iOS Settings privacy link must point to the live FoxDesk privacy document.');
$assert(str_contains($iosSettingsView, 'https://foxdesk.net/index.php?page=legal&type=terms'), 'iOS Settings terms link must point to the live FoxDesk terms document.');
...
$assert(str_contains($iosSettingsView, 'Push diagnostics'), 'iOS Settings must include debug/staging APNs diagnostics for physical-device smoke testing.');
$assert(str_contains($iosSettingsView, 'session.client.environment.baseURL.absoluteString'), 'iOS debug Settings must show the active API base URL for TestFlight/backend verification.');
$assert(str_contains($iosSettingsView, 'Copy APNs token'), 'iOS Settings must let testers copy the APNs token for live smoke tests.');
$assert(str_contains($iosSettingsView, 'UIPasteboard.general.string'), 'iOS Settings must copy the APNs token to the clipboard.');
$assert(str_contains($iosSettingsView, '.textSelection(.enabled)'), 'iOS Settings must make APNs token text selectable.');
$assert(str_contains($iosSettingsView, '#if DEBUG'), 'iOS APNs token diagnostics must be limited to debug/staging builds.');
...
$assert(str_contains($iosScreenshotDemoRootView, 'Privacy Policy'), 'iOS screenshot Settings fixture must show the privacy policy row.');
$assert(str_contains($iosScreenshotDemoRootView, 'Terms'), 'iOS screenshot Settings fixture must show the terms row.');
$assert(str_contains($iosScreenshotDemoRootView, 'Request account deletion'), 'iOS screenshot Settings fixture must show the account deletion request row.');
