...
| Photos, files, and previews | `POST /api/mobile/v1/attachments`, `GET /api/mobile/v1/attachments/{id}`, mobile Bearer authorization on protected download/preview URLs | `AttachmentUploadSection`, `CameraCaptureView`, `AttachmentPreviewView` | `FoxDeskAPIClientTests`, `tests/mobile-api-contract-test.php`, `tests/r2-storage-contract-test.php`, local write smoke |
