...
| New ticket from iPhone | `GET /api/mobile/v1/tickets/create-options`, `POST /api/mobile/v1/tickets` | `NewTicketView` from the bottom New ticket tab and Tickets flow | `FoxDeskAPIClientTests`, `tests/mobile-api-contract-test.php` |
...
| Photos, files, and previews | `POST /api/mobile/v1/attachments`, `GET /api/mobile/v1/attachments/{id}`, `GET /api/mobile/v1/attachments/{id}/download` | `AttachmentUploadSection`, `CameraCaptureView`, `AttachmentPreviewView` | `FoxDeskAPIClientTests`, `tests/mobile-api-contract-test.php`, `tests/r2-storage-contract-test.php`, live write smoke with authorized download |
| Push notifications | `POST /api/mobile/v1/device-token`, `POST /api/mobile/v1/device-token/unregister`, APNs payload with `ticket_id` + `ticket_hash` | `PushRegistrationService`, `NotificationsView`, `PushNavigationRouter` with hash fallback | `PushRegistrationServiceTests`, `PushNavigationRouterTests`, `tests/ios-mvp-endpoint-matrix-contract-test.php`, `bin/test-apns-push.php` |
...
| Offline and speed fallback | cached API payloads and local-only draft state | `DashboardView`, `TicketsView`, `TicketDetailView`, `TicketComposerView`, `NewTicketView`, `TicketAttachmentsView` using `HomeFeedCacheStore`, `TicketListCacheStore`, `TicketDetailCacheStore`, `TicketCommentDraftStore`, `ProtectedLocalCache`, and `StagedAttachmentUploadState`; per-user files use complete file protection, cache expiry, legacy migration, and logout cleanup | `FoxDeskAPIClientTests`, `tests/mobile-api-contract-test.php`, `npm run ios:gate`, `npm run ios:beta:gate` |
| Lightweight account/logout | `GET /api/mobile/v1/me`, refresh-token-authorized `POST /api/mobile/v1/logout`, account links | `AccountView`, `AppSession` generation guard and transient restore preservation | `FoxDeskAPIClientTests`, `tests/ios-mvp-scope-contract-test.php`, `npm run ios:gate` |
| Reliable mobile writes | per-session `Idempotency-Key` reservation and response replay for authenticated writes | `FoxDeskAPIClient` retries one lost write response with the same key | `testMobileWriteRetriesOnceWithTheSameIdempotencyKey`, `tests/mobile-idempotency-contract-test.php`, `npm run ios:gate` |
