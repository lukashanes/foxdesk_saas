...
if demo_write_ready "$DEMO_EVIDENCE"; then
  mark_human_gate "Demo reviewer write proof" "ready" "passing demo evidence includes ticket creation plus an internal comment-with-time reload check"
elif [[ "${FOXDESK_IOS_DEMO_WRITE:-}" == "1" ]]; then
  mark_human_gate "Demo reviewer write proof" "missing" "FOXDESK_IOS_DEMO_WRITE=1 is set, but demo evidence does not include created ticket plus linked comment/time reload proof"
else
