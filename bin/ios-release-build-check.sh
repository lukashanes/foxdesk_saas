...
grep -q 'Release' "$PROJECT_FILE" || fail "Generated project must contain a Release configuration."
grep -q 'APS_ENVIRONMENT = production;' "$PROJECT_FILE" || fail "Release config must use production APNs entitlement."
grep -Eq 'FOXDESK_API_BASE_URL = "?https://app\.foxdesk\.net/index\.php"?;' "$PROJECT_FILE" || fail "Release config must use production app.foxdesk.net API base."
...
log "Building Release for ${DESTINATION}"
(cd "$IOS_DIR" && xcodebuild -project FoxDesk.xcodeproj -scheme FoxDesk -configuration Release -destination "$DESTINATION" CODE_SIGNING_ALLOWED=NO -quiet build)
