...
grep -q 'Staging' "$PROJECT_FILE" || fail "Generated project must contain a Staging configuration."
grep -q 'APS_ENVIRONMENT = development;' "$PROJECT_FILE" || fail "Staging config must use development APNs entitlement."
grep -Eq 'FOXDESK_API_BASE_URL = "?https://staging\.app\.foxdesk\.net/index\.php"?;' "$PROJECT_FILE" || fail "Staging config must use staging.app.foxdesk.net API base."
...
log "Building Staging for ${DESTINATION}"
(cd "$IOS_DIR" && xcodebuild -project FoxDesk.xcodeproj -scheme FoxDesk -configuration Staging -destination "$DESTINATION" CODE_SIGNING_ALLOWED=NO -quiet build)
