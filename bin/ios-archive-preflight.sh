...
INFO_PLIST="$IOS_DIR/FoxDesk/Info.plist"
...
[[ -f "$INFO_PLIST" ]] || fail "Missing explicit app Info.plist."
...
contains "$MANIFEST" "INFOPLIST_FILE: FoxDesk/Info.plist" "App target must use the explicit release Info.plist."
contains "$INFO_PLIST" '<key>FOXDESK_API_BASE_URL</key>' "Info.plist must declare the API base key."
contains "$INFO_PLIST" '<string>$(FOXDESK_API_BASE_URL)</string>' "Info.plist must resolve the API base from the build configuration."
...
contains "$INFO_PLIST" '<key>ITSAppUsesNonExemptEncryption</key>' "App must declare its export-compliance encryption setting."
contains "$INFO_PLIST" '<false/>' "App must declare that it does not use non-exempt encryption."
...
contains "$BUILD_SETTINGS" "FOXDESK_API_BASE_URL = https://app.foxdesk.net/index.php" "Production archive must point to production app.foxdesk.net."
contains "$BUILD_SETTINGS" "INFOPLIST_FILE = FoxDesk/Info.plist" "Production archive must use the explicit app Info.plist."
